...
public class CityTable extends AbstractTable<AbstractRow>{
	...
	public void addRow (String cityName, String cityID, String cityPop) throws TableRowLimitException {
		...
		try {
			int intCityID = Integer.parseInt(cityID);
			double doubleCityPop = Double.parseDouble(cityPop);
			...
			if (currentIndex == 100) {
				...
				this.getTableRows().add(currentIndex, cr);	
				incrementTableIndex();
			}
	...
	public String removeRow(String cityID) throws RowNotFoundException{
		...
		for (int i= 0; i < getTableIndex(); i++) {
			String[] currentRow = ((CityRow) getTableRows().get(i)).getSingleCityRow();
